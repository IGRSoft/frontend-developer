#!/usr/bin/env bash
#
# validate.sh — release-gate validator for the frontend-developer plugin.
#
...
#
# Gates:
#   1-2  manifests parse; every declared agent/command path exists; every
#        skill entry is a directory with SKILL.md; hook scripts resolve.
#   3    orphan detection (every agents/*.md except _base/ and every
#        commands/*.md is listed in the marketplace manifest).
#   4    command frontmatter (description + argument-hint + allowed-tools,
#        whitelist-checked).
#   5    skill frontmatter (name + description; trigger lint; >8KB WARN;
#        backticked references/templates resolve).
#   6    agent frontmatter (name + description + model; trigger lint;
#        in-repo name uniqueness; external-collision scan over installed
#        plugin caches).
#   7    subagent_type prefix gate; own-plugin targets must exist.
#   8    review-only marker gate (the 3 fe-* review agents carry
#        disallowed-tools: Write, Edit and never list Write/Edit in tools).
#   9    version-marker grep (every version-specific skill links the
#        canonical _shared/version-feature-matrix.md).
#   10   graceful-degradation grep (every command prints an install hint
#        and skips on a missing optional tool).
#
# Usage:
...
# This plugin's own name — used to skip its cached copies during the
# external-collision scan and to gate own-plugin subagent_type targets.
OWN_PLUGIN="frontend-developer"
...
if [[ -d commands ]]; then
	while IFS= read -r f; do
		...
		if [[ -z "${tools}" ]]; then
			...
			# entry may be a bare tool name or a scoped form (e.g. Bash(npm:*));
...
if [[ -d skills ]]; then
	while IFS= read -r skill; do
		...
		# Body > 8KB without a references/ sibling -> warning. The 8KB rule is a
...
# plugin's own cache copies (anything under a frontend-developer path) so a
...
if [[ -d agents ]]; then
	while IFS= read -r f; do
		...
		[[ -n "${ext}" ]] && err "${rel}" "agent name '${aname}' collides with ${ext}" "rename to an fe-prefixed or otherwise unique name"
	done < <(find agents -type f -name '*.md')
fi
...
ALLOWED_PREFIX_RE='^(frontend-developer|backend-developer|apple-developer|corpflow|security-scanning|debugging-toolkit|general-purpose)'
	...
	while IFS= read -r f; do
		...
		while IFS= read -r st; do
			...
			if [[ ! "${st}" =~ ${ALLOWED_PREFIX_RE} ]]; then
				err "${rel}" "subagent_type '${st}' has a disallowed plugin prefix" "use frontend-developer:/backend-developer:/apple-developer:/corpflow:/security-scanning:/debugging-toolkit:/general-purpose:"
				continue
			fi
			# Own-plugin targets must exist as agent files.
			case "${st}" in
				"${OWN_PLUGIN}":*)
					target="${st#"${OWN_PLUGIN}":}"
...
# ---------------------------------------------------------------------------
# 8. Review-only marker gate
# ---------------------------------------------------------------------------

# The three review-only specialists must carry `disallowed-tools: Write, Edit`
# and must NOT list Write or Edit in their tools: line.
REVIEW_ONLY_AGENTS="fe-performance-engineer fe-accessibility-auditor fe-security-auditor"

for a in ${REVIEW_ONLY_AGENTS}; do
	f="agents/${a}.md"
	if [[ ! -f "${f}" ]]; then
		err "${f}" "expected review-only agent file missing" "create agents/${a}.md with disallowed-tools: Write, Edit"
		continue
	fi
	dis="$(fm_field "${f}" "disallowed-tools")"
	if [[ "${dis}" != *"Write"* || "${dis}" != *"Edit"* ]]; then
		err "${f}" "review-only agent missing 'disallowed-tools: Write, Edit'" "add 'disallowed-tools: Write, Edit' before the tools: key"
	fi
	atools="$(fm_field "${f}" tools)"
	IFS=',' read -ra tentries <<<"${atools}"
	for entry in "${tentries[@]}"; do
		tbase="$(printf '%s' "${entry}" | sed -E 's/^[[:space:]]+//; s/[[:space:]]+$//; s/\(.*$//')"
		if [[ "${tbase}" == "Write" || "${tbase}" == "Edit" ]]; then
			err "${f}" "review-only agent lists '${tbase}' in tools:" "remove ${tbase} from the tools: line (review-only)"
		fi
	done
done

# ---------------------------------------------------------------------------
# 9. Version-marker gate
# ---------------------------------------------------------------------------

# Every skill that makes a version-specific claim must link the canonical
# version-feature matrix. We assert the canonical file exists and that the
# marker token appears across the skill tree.
VERSION_MATRIX="skills/_shared/version-feature-matrix.md"
...
# ---------------------------------------------------------------------------
# 10. Graceful-degradation gate
# ---------------------------------------------------------------------------

# Every command must document a missing-tool fallback (install hint + skip,
# never hard-fail). We look for an install-hint cue in the command body.
if [[ -d commands ]]; then
	while IFS= read -r f; do
		rel="${f#./}"
		if ! grep -qiE 'not installed|install hint|skip this step|never (exit|fail)|npm install|npx ' "${f}"; then
			warn "${rel}" "no graceful-degradation cue (install hint / skip on missing tool)" "document a missing-tool fallback: print an install hint and skip, never hard-fail"
		fi
	done < <(find commands -type f -name '*.md')
fi
