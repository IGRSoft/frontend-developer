#!/usr/bin/env bash
# PreCompact → state checkpoint (frontend-developer plugin, v1.0.0+).
# Copies .context/state.json to .context/state.checkpoint-<ts>.json before
# auto-compaction so long frontend-developer sessions survive context summarization.
...
if [ ! -f "$STATE_FILE" ]; then
  if command -v jq >/dev/null 2>&1; then
    jq -cn --arg ts "$(date -u +%FT%TZ)" '{
      ts: $ts,
      actor: "frontend-developer:hook:precompact",
