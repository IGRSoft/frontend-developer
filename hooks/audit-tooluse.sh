#!/usr/bin/env bash
# PostToolUse → audit.jsonl writer (frontend-developer plugin, v1.0.0+).
...
#
# ADVISORY ROW: frontend-developer agents run as subagents under the corpflow
...
# frontend-developer runs standalone (no corpflow), these rows stand alone.
#
...
ROW=$(printf '%s' "$PAYLOAD" | jq -c \
  --arg ts "$(date -u +%FT%TZ)" \
  --arg actor "frontend-developer:hook:audit-tooluse" \
  --arg kind "$KIND" '
  {
    ts: $ts,
    actor: $actor,
...
if [ "$SELF_TEST" -eq 1 ]; then
  printf '%s\n' "$ROW" | jq -e '.metadata.dedupe_key == "sess_test:toolu_test" and .metadata.duration_ms == 42 and .metadata.effort == "medium" and .metadata.advisory == true and .actor == "frontend-developer:hook:audit-tooluse"' >/dev/null \
