...
3. Default stage: **SR context provider** — `igrsoft:security-reviewer` owns `.context/security-review.md`; this agent supplies front-end-specific findings (XSS, CSP, injection sinks, bundle secrets, supply chain, SSRF) as input for that agent to merge
...
- Cross-check CVE findings with `frontend-developer:fe-dependency-manager` for the actual one-at-a-time remediation, and with `igrsoft:security-reviewer` for the SR stage.
