...
When `.context/state.json` exists, this agent is inside an company-workflow workflow. Follow `_base/frontend-agent.md § Workflow Stage Participation § DV Stage` for the contract; the router-specific steps:
