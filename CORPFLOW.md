...
This is the **only** file in frontend-developer that knows corpflow exists. Delete it and the plugin is
...
`.context/state.json` exists → yes. Otherwise every rule below is inert and frontend-developer behaves exactly
...
### Stages owned by frontend-developer

frontend-developer owns these stages when dispatched:
...
| DR (Developer Review) | `fe-code-fixer` | `metadata.gate_blockers[]` + minimal-diff remediation |
| SR (Security) | `fe-security-auditor` | development context + web front-end security checklist |
| QA (Quality) | `fe-test-generator` | development context + test requirements |
...
Build and test **only** through `/frontend-developer:build-test`. Do not invoke the toolchain
...
```yaml
handoff:
  from: "frontend-developer:react-developer"
...
```yaml
handoff:
  from: "frontend-developer:frontend-architector"
  to: "corpflow:software-architector"
  stage: "AR"
  run_index: <N>
  status: "completed"
  verdict: "pass"
  artifacts: [".context/frontend-architecture.md"]
  metadata:
    patterns_selected: []
    constraints: []
```
...
```yaml
handoff:
  from: "frontend-developer:<support-agent>"
  to: "frontend-developer:react-developer"
  stage: "DV"
  run_index: <N>
  status: "completed"
  verdict: "pass"
...
When a corpflow contract changes, this file changes and nothing else in frontend-developer does. That property
...
- Hooks that describe orchestrator interop say "the orchestrator" generically. They work under
  ...
  be coupled.
- frontend-developer's own version does not track corpflow's. Record the targeted corpflow version below and
