...
# corpflow Integration — frontend-developer

The only file in frontend-developer that knows corpflow exists — delete it and the plugin is
...
### Stages frontend-developer works in

| Stage | frontend-developer agent | Handoff data |
|---|---|---|
| AR (Architecture) | `frontend-architector` | planning context + platform constraints — **consultation only**, corpflow retains the stage |
| DV (Development) | `frontend-developer`, `react-developer`, `vue-developer`, `svelte-developer`, `angular-developer`, `typescript-developer`, `css-developer` | planning + architecture context |
| DR (Developer Review) | `fe-code-fixer` | `metadata.gate_blockers[]` + minimal-diff remediation — **consultation only**, corpflow retains the stage and writes the artifact |
| SR (Security) | `fe-security-auditor` | development context + web front-end security checklist — **consultation only**, corpflow retains the stage and writes the artifact |
| QA (Quality) | `fe-test-generator` | development context + test requirements — **consultation only**, corpflow retains the stage and writes the artifact |
| DV-support | `fe-accessibility-auditor` (QA-support), `fe-performance-engineer`, `fe-dependency-manager` | scoped findings returned to the parent DV agent, which owns the artifact |

**DV is the only stage ownership transfers for.** Read `tasks.DV0.agent`: a `frontend-developer:` id
...
| Field | Value |
|---|---|
| `requires_screenshots` default | `true` |
| Build Evidence adapter | `web_adapter (Playwright / Chrome MCP) + Lighthouse/axe` |
| Error file | `.context/errors/react-developer.md` (basename of the agent file, not the qualified id) |

Build Evidence is Playwright or Chrome MCP captures plus Lighthouse and axe reports. When `metadata.ui_visual_check: true`, a Storybook story, an isolated component screenshot, or a route screenshotted without interaction does **not** satisfy DV exit — drive the running app.

## Build and test

- Build and test **only** through `/frontend-developer:build-test`; never invoke the toolchain directly.
...
| Stage | Artifact |
|---|---|
| AR | `.context/frontend-architecture.md` (consultation output, ≤500-token return summary) |
...
| Stage | Delta | Verdict |
|---|---|---|
| AR | `key_decisions` + `open_questions` anchored in `frontend-architecture.md`; no `files_touched` / `worktree` / `architecture` | ok / blocked / escalate |
...
AR writes `.context/frontend-architecture.md` and returns ≤500 tokens for `corpflow:software-architector` to merge.
...
```markdown
## <STAGE> Summary — frontend-developer:<agent>
...
- frontend-developer's version does not track corpflow's. Bump the target below when the contract changes.
