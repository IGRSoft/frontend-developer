# DV Screenshot Manifest Template (igrsoft v3.12.0)
...
(default **TRUE** for web UI changes). If this file is absent at `SubagentStop`, igrsoft's
...
This mirrors igrsoft's `dv-screenshot-capture` skill output so the gate finds it, and the
`design_ref` column is the join key for QA's RMSE design-diff (igrsoft 3.11.4).
...
2. Capture each meaningful **route/state** via igrsoft's `web_adapter`:
